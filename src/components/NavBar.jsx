...
const NavBar = () => {
  ...
  return (
    <nav className='fixed top-0 left-0 right-0 z-50 flex justify-between items-center bg-gradient-primary shadow-sm'>
      <div className={`w-full mx-auto flex justify-between items-center max-w-[1280px] h-[80px] px-6 z-50 transition-all duration-200 ease-in-out ${isMenuOpen ? 'border-b border-gray-200' : 'border-b border-transparent'}`}>
        <div className='flex items-center gap-6'>
          <Link to='/' className='text-3xl font-bold uppercase flex items-center gap-1'>
          ...
          <ul className='flex items-center gap-4 max-md:hidden'>
            ...
            <li>
              <Link 
                to='/subjects' 
                className={`hover:text-primary cursor-pointer transition-colors duration-300 ${
                  isActiveLink('/subjects') ? 'text-primary font-semibold' : ''
                }`}
              >
                Subjects
              </Link>
        ...
        <div className='flex items-center gap-4'>
          <a 
            ...
            className='bg-primary text-white px-4 py-2 rounded-full max-md:hidden hover:bg-primary/70 transition-colors duration-300 cursor-pointer'
          >
            Enroll Now
          </a>
          ...
          >
            <div className='w-6 h-5 relative flex flex-col justify-evenly gap-1'>
              <span className={`w-full h-0.5 bg-black transform transition-all duration-100 origin-center ${isMenuOpen ? 'opacity-0' : ''}`} />
              <span className={`absolute w-full h-0.5 bg-black transform transition-all duration-300 origin-center ${isMenuOpen ? '-rotate-45' : ''}`} />
              <span className={`absolute w-full h-0.5 bg-black transform transition-all duration-300 origin-center ${isMenuOpen ? '-rotate-[-45deg]' : ''}`} />
              <span className={`w-full h-0.5 bg-black transform transition-all duration-100 origin-center ${isMenuOpen ? 'opacity-0' : ''}`} />
            </div>
          </button>
        </div>
        ...
        <div className='flex flex-col mt-20 h-full '>
          <ul className='flex flex-col'>
            ...
            >
              <Link 
                className={`text-lg font-medium block w-full h-full ${
                  isActiveLink('/privacy') ? 'text-primary font-semibold' : ''
                }`} 
                to='/privacy'
                onClick={handleCloseMenu}
              >
            ...
            >
              <Link 
                className={`text-lg font-medium block w-full h-full ${
                  isActiveLink('/legal') ? 'text-primary font-semibold' : ''
                }`} 
                to='/legal'
                onClick={handleCloseMenu}
              >
            ...
            <li className='p-6 text-center'>
              <div className='flex items-center justify-center gap-2'>
                <a href='https://www.instagram.com/taiyo_tuition' target='_blank' rel='noopener noreferrer'>
